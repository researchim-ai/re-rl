...
class BaseTask(DifficultyMixin, metaclass=TaskMeta):
    ...
    # ------------------------------------------------------------------
    # Верификация ответа модели (хук для RL / оценки)
    # ------------------------------------------------------------------
    def verify(self, prediction: str) -> float:
        """Оценивает ответ модели относительно эталона.

        Возвращает награду в диапазоне ``[0, 1]``. По умолчанию делегирует
        общей логике сравнения из :mod:`re_rl.rewards`, используя
        ``final_answer`` как эталон.

        Подклассы могут переопределить метод для задач с несколькими
        корректными решениями (например, латинский квадрат) или со
        специфической логикой сравнения.

        Args:
            prediction: Текст ответа модели (может содержать ``<answer>`` теги).

        Returns:
            Награда от 0.0 до 1.0.
        """
